...
var  peopleData = require('./dummyData.json');

var oEvent =  new Event(7,'10/08/2016',peopleData);
...
describe('formatted data', function(){
...
describe('upcoming events',function(){
    it('should get one birthday',function(){
        var result = oEvent.upcomingEvents(); 
        should(result.birthdays.length).be.exactly(1);
    });


    it('should get one anniversary',function(){
        var result = oEvent.upcomingEvents(); 
        should(result.anniversaries.length).be.exactly(1);
    });
